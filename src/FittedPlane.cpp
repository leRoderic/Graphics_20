...
FittedPlane::FittedPlane(vec3 limit1, vec3 limit2, vec3 normal, float d, float v) : Plane(normal, limit1, v) {
    //this->normal = normal;
    this->p1 = limit1;
    this->p2 = limit2;
}
...
bool FittedPlane::intersection(const Ray &raig, float t_min, float t_max, IntersectionInfo &info) const {
    if (Plane::intersection(raig, t_min, t_max, info)) {
        if (info.p.x <= p2.x && info.p.x >= p1.x && info.p.z <= p2.z && info.p.z >= p1.z) {
            return true;
        }
    }
    return false;
